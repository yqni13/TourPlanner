    ...
    {        
        public static void AddTourLog(TourLogs log)
        {
            using (IDbConnection connection = DBConnection.GetConnection())
            {
                ...
                command.CommandText = $"INSERT INTO tourlogs (tl_id, tl_tour, tl_date, tl_time, tl_difficulty, tl_comment, tl_rating)" +
                    $"VALUES (@tl_id, @tl_tour, @tl_date, @tl_time, @tl_difficulty, @tl_comment, @tl_rating)";
        ...
        public static void UpdateTourLog(TourLogs log)
        {
            ...
            using (IDbConnection connection = DBConnection.GetConnection())
            {
                ...
                command.CommandText = $"UPDATE {tableName} SET tl_time=@time, tl_difficulty=@diff, tl_comment=@cmnt, tl_rating=@rating WHERE tl_id = @logId";
